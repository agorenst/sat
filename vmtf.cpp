...
void vmtf_t::invariant() {
  ...
  if (!std::none_of(std::reverse_iterator(std::next(next_to_choose)), std::rend(q),
                      [&](variable_t v) {
                        return trail.literal_unassigned(lit(v)) && trail.level(lit(v)); })) {
    debug();
    assert(0);
  }
}
...
variable_t vmtf_t::choose() {
  invariant();
  auto it = std::find_if(std::begin(q), std::end(q), [&](variable_t v) { return trail.literal_unassigned(lit(v)); });
...
void vmtf_t::unassign(variable_t v) {
  ...
  if (s > ns) {
    ...
    assert(std::find(std::reverse_iterator(next_to_choose), std::rend(q), v) != std::rend(q));
